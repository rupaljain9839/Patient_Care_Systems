...
from services.prescription_service import create_prescription, get_doctor_prescriptions, generate_prescription_pdf
from services.pharmacy_service import list_medicines, CATEGORIES as MED_CATEGORIES
from services.lab_service import (
    get_pending_bookings_for_patient,
    create_lab_report,
    get_doctor_reports,
    generate_lab_report_pdf,
)
from views.pharmacy_view import _medicine_card_html
...
def _sidebar(user):
    ...
    with st.sidebar:
        ...
        if st.button("💊 Pharmacy", use_container_width=True):
            st.session_state["doctor_nav"] = "pharmacy"
            st.rerun()
...
def _render_write_prescription(user):
    render_page_header("Write Prescription", "Issue a new prescription for a patient", badge_text="PCMS-HS")
    ...
    st.write("**Medicines**")
    catalog_names = sorted({m["name"] for m in list_medicines(active_only=True)})
    CUSTOM_OPTION = "✏️ Other (type manually)"
    med_select_options = [""] + catalog_names + [CUSTOM_OPTION]

    for i, item in enumerate(st.session_state["rx_items"]):
        c1, c2, c3, c4, c5 = st.columns(5)
        with c1:
            current_name = item["medicine_name"]
            if current_name in catalog_names:
                default_choice = current_name
            elif current_name:
                default_choice = CUSTOM_OPTION
            else:
                default_choice = ""
            choice = st.selectbox(
                "Medicine", med_select_options,
                index=med_select_options.index(default_choice),
                key=f"rx_med_select_{i}",
            )
            if choice == CUSTOM_OPTION:
                item["medicine_name"] = st.text_input(
                    "Custom medicine name",
                    value=current_name if current_name not in catalog_names else "",
                    key=f"rx_med_custom_{i}",
                    placeholder="Type medicine name",
                    label_visibility="collapsed",
                )
            else:
                item["medicine_name"] = choice
        with c2:
    ...
    if not issued:
        ...
        for p in issued:
            ...
            with c2:
                ...
                with col_view:
                    if st.button("👁 View", key=f"doc_presc_view_{p['id']}", use_container_width=True):
                        st.session_state[f"doc_show_presc_pdf_{p['id']}"] = not st.session_state.get(
                            f"doc_show_presc_pdf_{p['id']}", False
                        )
                        st.rerun()
                with col_dl:
                    st.download_button(
                        ...
                        key=f"doc_presc_pdf_{p['id']}", use_container_width=True,
                    )
            if st.session_state.get(f"doc_show_presc_pdf_{p['id']}", False):
                render_pdf_inline(generate_prescription_pdf(p["id"]))


def _render_issue_lab_report(user):
    render_page_header("Issue Lab Report", "Enter results for a patient's lab test", badge_text="PCMS-HS")
    ...
    st.divider()
    st.subheader("Lab reports you've issued")
    issued_reports = get_doctor_reports(user["id"])
    if not issued_reports:
        st.info("No lab reports issued yet.")
    else:
        for r in issued_reports:
            c1, c2 = st.columns([4, 2])
            with c1:
                st.write(f"**{r['patient_name']}** — {r['test_name']} · {r['created_at'].strftime('%d %b %Y') if r['created_at'] else ''}")
            with c2:
                col_view, col_dl = st.columns(2)
                with col_view:
                    if st.button("👁 View", key=f"doc_lab_view_{r['id']}", use_container_width=True):
                        st.session_state[f"doc_show_lab_pdf_{r['id']}"] = not st.session_state.get(
                            f"doc_show_lab_pdf_{r['id']}", False
                        )
                        st.rerun()
                with col_dl:
                    st.download_button(
                        ...
                        key=f"doc_lab_pdf_{r['id']}", use_container_width=True,
                    )
            if st.session_state.get(f"doc_show_lab_pdf_{r['id']}", False):
                render_pdf_inline(generate_lab_report_pdf(r["id"]))


def _render_pharmacy_readonly(user):
    render_page_header("Pharmacy", "Browse the medicine catalog — view only", badge_text="PCMS-HS")
    st.caption("Doctors can view stock and pricing here but can't place orders — that's patient-only.")

    category = st.pills(
        "Category", ["All"] + MED_CATEGORIES,
        selection_mode="single", default="All", key="doc_pharmacy_category_pill",
    )
    category = category or "All"

    col_search, col_sort = st.columns([3, 1])
    with col_search:
        search = st.text_input("Search medicine", placeholder="🔍 Search medicines", key="doc_pharmacy_search")
    with col_sort:
        sort_by = st.selectbox(
            "Sort by", ["Relevance", "Price: Low to High", "Price: High to Low", "Name: A-Z"],
            key="doc_pharmacy_sort",
        )
    ...
    st.caption(f"{len(medicines)} medicine{'s' if len(medicines) != 1 else ''} in catalog")
...
def render_doctor_portal():
    ...
    if nav == "pharmacy":
        _render_pharmacy_readonly(user)
        return
    ...
    render_page_header(user["full_name"], subtitle, badge_text="PCMS-HS")
