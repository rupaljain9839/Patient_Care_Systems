...
from services.prescription_service import get_all_prescriptions
...
def _render_doctors_management():
    ...
    render_page_header("Doctors", "View, update, or remove doctors in the hospital", badge_text="IPCMS")
...
def _render_console():
    render_page_header("Admin Console", "Full oversight of IPCMS", badge_text="IPCMS")
...
def _medicines_tab():
    ...
    if submitted:
        result = add_medicine(name, category, description, price, int(stock), icon)
        if result.ok:
    ...
    for m in medicines:
        ...
        render_html(f"""
            <div style="background:white; border-radius:14px; padding:1rem 1.3rem; margin-bottom:0.6rem;
                        box-shadow:0 2px 10px rgba(20,108,148,0.1); border:1px solid #d7edf5;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div>{m['icon']} <b style="color:#0b3d5c;">{m['name']}</b>
        ...
        with st.expander(f"Edit — {m['name']}"):
            with st.form(f"edit_medicine_{m['id']}"):
                ...
                with col2:
                    ...
                    new_icon = st.text_input("Icon", value=m["icon"], max_chars=4, key=f"med_icon_{m['id']}")
                save = st.form_submit_button("Save changes", use_container_width=True)

            if save:
                result = update_medicine(m["id"], price=float(new_price), stock=int(new_stock), category=new_category, icon=new_icon)
                if result.ok:
                    st.success(result.message)
                    st.rerun()
                else:
...
def _prescriptions_oversight_tab():
    ...
    for p in prescriptions:
        st.write(f"**{p['patient_name']}** — Dr. {p['doctor_name']} · {p['diagnosis']} · {p['item_count']} medicine(s)")
...
def _render_pharmacy():
    render_page_header("Pharmacy", "Manage the medicine and lab test catalog", badge_text="IPCMS")
