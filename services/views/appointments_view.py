...
def _calendar_tab(patient_id: int):
    if "appt_week_offset" not in st.session_state:
        st.session_state["appt_week_offset"] = 0

    nav1, nav2, nav3 = st.columns([1, 1, 4])
    with nav1:
        if st.button("◀ Previous", use_container_width=True):
            st.session_state["appt_week_offset"] -= 1
            st.rerun()
    with nav2:
    ...
    today = date.today()
    week_start = today - timedelta(days=today.weekday()) + timedelta(weeks=st.session_state["appt_week_offset"])
    ...
    for i, d in enumerate(days):
        with cols[i]:
            ...
            if day_appts:
                chips = "".join(
                    compact_html(f"""
                        <div style="background:#dff3fa; border-radius:8px; padding:0.45rem 0.55rem; margin-bottom:0.4rem; font-size:0.75rem;">
                            <b>{a['start_time'].strftime('%H:%M')}</b><br>{a['doctor_name']}<br>
                ...
                chips = '<div style="color:#a5707a; font-size:0.72rem; padding-top:0.3rem;">—</div>'

            render_html(f"""
                <div style="{header_style} border-radius:10px; padding:0.5rem; text-align:center;
                            font-weight:800; font-size:0.78rem; margin-bottom:0.5rem;">
...
def render_appointments_view(user):
    render_page_header("Appointments", "Book a slot and track your visits", badge_text="IPCMS")
